...
// Today's best run (highest rings, earliest submission on ties), across all players.
async function fetchTodaysBest() {
  try {
    ...
    const url = `${SUPABASE_URL}/rest/v1/runs?select=rings,mode,coach_name,players,created_at`
      ...
      + `&order=rings.desc,created_at.asc&limit=1`;
...
export default function SixRings() {
  ...
  const [todaysBest, setTodaysBest] = useState(null);
  ...
  return (
    <div className="wrap">
      <style>{`
        ...
        .todaysBest { background: rgba(217,164,65,0.08); border: 1px solid rgba(217,164,65,0.35); border-radius: 10px; padding: 14px 18px; margin-bottom: 18px; text-align: center; }
        ...
        .todaysBest .tbNames { font-size: 14px; color: #EDE6D6; margin-bottom: 4px; }
      ...
      <div className="stage">
        ...
        {phase === "modeSelect" && todaysBest && (
          <div className="todaysBest">
            <div className="tbLabel">Today's Best &mdash; All Players</div>
            <div className="tbNames">{todaysBest.players.map((p) => p.name.split(" ").pop()).join(", ")}</div>
            <div className="tbMeta">
              {todaysBest.rings} ring{todaysBest.rings === 1 ? "" : "s"} under {todaysBest.coach_name}
            </div>
          </div>
        )}
